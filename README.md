...
single-cell RNA-seq data, based on a linear-based variability metric and a corresponding differential testing framework.
...
- Works for both balanced and unbalanced group sizes
...
4. Run Nutkin to quantify variability and perform differential testing
5. Interpret the output results

### Required input

- **scRNA-seq data(.h5ad format)**: Data can be provided either as a preprocessed or raw dataset. If raw data are supplied, Nutkin will perform preprocessing automatically.
...
- Scree plot (`--run_pca`): Shows the variance explained by each principal component.
  ...
  or the number of PCs explaining sufficient variance. The default value is 50.
- Embedding plot (`--run_embedding`): UMAP visualizations for exploratory analysis.
...
- `all_pairwise_scatter.png`, `all_pairwise_kde.png`, `all_pairwise_both.png`: PCA scatter plots and contour plots (default: top 3 PCs)
...
| `plot_type`          | str      | 'scatter'       | Type of PCA visualization: `"scatter"`, `"contour"`, or `"both"`                                               |
...
- `differential_test()` performs pairwise statistical comparisons between groups and saves `differential_variability.csv`.
  ...
  - If `group1` and `group2` are `None`, the function automatically compares the variability of adjacent groups according to the order of variability in `var_measurement.csv`. 
...
## User manual and example test

The full user manual and example test notebooks can be found in the `Nutkin/manual` and `Nutkin/test` directories, respectively.
